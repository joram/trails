...
_GEOHASH_PREFIX_RE = re.compile(r"^[0-9b-hjkmnp-z]{5,32}$")
...
def feature_collection_for_prefix(data_dir: str | Path, prefix: str) -> FeatureCollection:
    ...
    Only scans the bucket directory for the first four characters of ``prefix``.
    """
    ...
    for json_path in sorted(bucket.glob("*.json")):
        try:
